...
class message(commands.Cog):
    ...
    async def check_for_afk(self, message:discord.Message):
        ...
        try:
            ...
            if guild_afk:
                await storage.afk.delete(guild_id=message.guild.id,user_id=message.author.id)
    ...
    async def check_afk_user_mention(self,message: discord.Message):
        ...
        try:
            ...
            for user in message.mentions:
                ...
                if global_afk:
                    await storage.afk.update(id=global_afk.get('id'),user_id=user.id,mentioned=global_afk.get('mentioned',0)+1)
                    created_at:datetime.datetime = global_afk.get('created_at')
                ...
                if guild_afk:
                    await storage.afk.update(id=guild_afk.get('id'),guild_id=message.guild.id,user_id=user.id,mentioned=guild_afk.get('mentioned',0)+1)
